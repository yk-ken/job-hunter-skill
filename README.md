...
**核心价值**：减少手动搜索和筛选岗位的时间，你只需查看不断增长的候选列表，挑选心仪岗位投递即可。

---

## 亮点

- **全自动搜索流程** — 搜索、详情获取、筛选、评分、记录，每 30 分钟自动运行一轮
...
| opencli | 网站 CLI 工具 | `npm install -g @jackwener/opencli` |
...
| 依赖 | 用途 | 安装 |
|------|------|------|
| [Claude Code](https://docs.anthropic.com/en/docs/claude-code) | 运行 Skill、创建定时任务 | `npm install -g @anthropic-ai/claude-code` |
| [opencli](https://github.com/jackwener/opencli) >= 1.5.0 | 搜索和获取 Boss 直聘岗位数据 | `npm install -g @jackwener/opencli` |
| Chrome + [Browser Bridge](https://github.com/jackwener/opencli) 扩展 | 复用 Chrome 登录态访问 Boss 直聘 | 安装 Chrome 扩展即可 |

首次运行 `/job-hunter` 时会自动检测以上环境，缺失项会给出具体安装指引。
